...
var appchainStateMap = map[g.EventType][]g.GovernanceStatus{
	...
	g.EventUpdate:          {g.GovernanceAvailable, g.GovernanceActiveFrozen, g.GovernancePassiveFrozen},
...
func setFSM(chain *Appchain, lastStatus g.GovernanceStatus) {
	chain.FSM = fsm.NewFSM(
		...
		fsm.Events{
			...
			// update 1
			{Name: string(g.EventUpdate), Src: []string{string(g.GovernanceAvailable), string(g.GovernanceActiveFrozen), string(g.GovernancePassiveFrozen), string(g.GovernanceFreezing), string(g.GovernanceLogouting)}, Dst: string(g.GovernanceUpdating)},
